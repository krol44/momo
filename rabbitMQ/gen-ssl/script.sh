...
openssl x509 -days 10000 -req -extfile <(printf "subjectAltName=DNS:*, DNS:alt.dns, DNS:host.docker.internal") -in /ssl/server.csr -CA /ssl/ca-cert.pem -CAkey /ssl/ca-key.pem -CAcreateserial -out /ssl/server-cert.pem

openssl req -new -nodes -text -sha256 -key /ssl/ca-key.pem -subj "/CN=*" > /ssl/client.csr -keyout /ssl/client-key.pem
openssl x509 -days 10000 -req -extfile <(printf "subjectAltName=DNS:*, DNS:alt.dns, DNS:host.docker.internal") -in /ssl/client.csr -CA /ssl/ca-cert.pem -CAkey /ssl/ca-key.pem -CAcreateserial -out /ssl/client-cert.pem